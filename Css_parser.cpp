...
void Css_parser::read_css()
{
    ...
    {
        selectors = false;
        if (input_char == input[input.size() - 1])
...
void Css_parser::handle_rest_of_commands()
{
    if (main_command == 'S')
    {
        ...
        if (command_part1_digit > 0 && command_part2_digit > 0)
        {
            ...
            if (section == nullptr)
                return;
            if (section->selectors_counter < command_part2_digit)
                return;
    ...
    if (main_command == 'D')
    {
        if (command_part2 == "*")
        {
            if (sections_list == nullptr)
                return;
            if (sections_list->number_of_active_sections() == 0)
                return;
        ...
        if (command_part2.size() > 0)
        {
            ...
            if (section->delete_property(command_part2))
            {
                ...
                if (section->block_data_counter == 0)
                    mainList::remove_section_index(command_part1_digit, last_list, sections_list);
            }
        }
    }
}
